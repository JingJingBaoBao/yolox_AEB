...
class YOLOLoss(nn.Module):    
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        weight = torch.tensor([1.0, 1.5, 0.])
        weight.requires_grad = False
        self.bcewithlog_loss_obj    = nn.BCEWithLogitsLoss(reduction="none")
        self.bcewithlog_loss_cls    = nn.BCEWithLogitsLoss(reduction="none", pos_weight=weight)
    ...
    def get_output_and_grid(self, output, k, stride): #获取中心点位置
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, outputs):
        ...
        loss_obj    = (self.bcewithlog_loss_obj(obj_preds.view(-1, 1), obj_targets)).sum()
        loss_cls    = (self.bcewithlog_loss_cls(cls_preds.view(-1, self.num_classes)[fg_masks], cls_targets)).sum()
        reg_weight  = 5.0
...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio = 0.05, warmup_lr_ratio = 0.01, no_aug_iter_ratio = 0.05, step_num = 10):
